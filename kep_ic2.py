...
class Disk:
    ...
    def get_velocities(self, x,y,z,G=1,M=1,H=0,mu=0,delta=0,**kargs):
        """
            GM = omega_K^2 r^3
            Sigma = r^mu
            c^2 = h^2 * r^delta
        ...
        self.omega = np.sqrt(G*M) / r1**(1.5)

        if kargs['ring']:
            grad = self.dprof_gauss_grad(r,phi,**kargs)
        else:
            grad = self.dprof_grad(r,phi,**kargs)

        self.omega *= (1 + H**2 * ( grad + delta))**(.5)

        self.vx = np.cos(phi) * self.vr - np.sin(phi) * self.omega*r
        self.vy = np.sin(phi) * self.vr + np.cos(phi) * self.omega*r
        return self.vx,self.vy,self.vz


    def dprof_gauss(self, r,phi,ri=1,rc=.35,**kargs):
        return np.exp(-((r-ri)/rc)**2)
    def dprof_gauss_grad(self, r,phi,ri=1,rc=1./np.sqrt(8.),**kargs):
        return -2*(r-ri)/rc**2

    def dprof(self, r,phi,mu=-.5,ri=.05,rc=2.,**kargs):
        res =   (1 - np.sqrt(ri/r)) * np.exp(-r/rc) * r**(mu)
        return res
    def dprof_grad(self, r,phi,mu=-.5,ri=.05,rc=2.,**kargs):
    # d ln Sigma/d ln r
        res = mu
        res += -r/rc
        res += 1./(-2 + 2*np.sqrt(r/ri))
        return res
    def get_density_pressure(self, x,y,z,H=0,delta=0,sig0=1,sig_floor=.001,**kargs):
        r,phi = get_polar_coords(x,y)
        if kargs['ring']:
            print(sig0)
            density = sig0*self.dprof_gauss(r,phi,**kargs)
            density += sig_floor
        else:
            density = sig0 * self.dprof(r,phi,**kargs)
            density += sig_floor
...
def makeIC_box(mydisk, **kargs):
    """
        mydisk is a Disk object, with the following functions:

        vx_g, vy_g, vz_g = mydisk.get_velocities(xv_g,yv_g,zv_g,**kargs)
        rho_g, pres_g = mydisk.get_density_pressure(xv_g,yv_g,zv_g,**kargs)

        Note:
        - Flag_DoublePrecision is now enabled by default.
    """
   ...
   # x0=np.arange(-.5,.5,1./N_1D); x0+=0.5*(0.5-x0[-1]);
    ...
    if kargs['rcenter']:
        # Kit's coordinate. r[0] is not rinner
        rbeta = (ro/ri)**(1.0/N_1D)
        r0 = 0.5*(1.0+rbeta) * ri * rbeta ** np.arange(N_1D) # "cell-centers"
        dr = (rbeta - 1.0) * ri * rbeta ** np.arange(N_1D) # dr
        # To get N_phi, we set dphi = 2pi/N = dr/r
        NPHI = np.ceil( 2.0*np.pi* 0.5*(1.0+rbeta) / (rbeta-1.0) )
        # phi coordinates
        p0 = np.linspace(0.0, 2.0*np.pi, NPHI, endpoint=False) 
        dphi = p0[1]-p0[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument('-soft',type=float,default=.0,help='Potential softening, (default=0)')
    ...
    parser.add_argument('--rcenter',action='store_true',help='Use center of the cell')
